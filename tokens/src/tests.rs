...
fn currency_adapter_burn_must_work() {
	ExtBuilder::default()
		.balances(vec![(ACEACCOUNT, DOT, 100)])
		.build()
		.execute_with(|| {
...
fn currency_adapter_reserving_balance_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		let _ = TreasuryCurrencyAdapter::deposit_creating(&ACEACCOUNT, 111);

		assert_eq!(TreasuryCurrencyAdapter::total_balance(&ACEACCOUNT), 111);
		assert_eq!(TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT), 111);
		assert_eq!(TreasuryCurrencyAdapter::reserved_balance(&ACEACCOUNT), 0);

		assert_ok!(TreasuryCurrencyAdapter::reserve(&ACEACCOUNT, 69));

		assert_eq!(TreasuryCurrencyAdapter::total_balance(&ACEACCOUNT), 111);
		assert_eq!(TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT), 42);
		assert_eq!(TreasuryCurrencyAdapter::reserved_balance(&ACEACCOUNT), 69);
	});
}
...
fn currency_adapter_balance_transfer_when_reserved_should_not_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(TreasuryCurrencyAdapter::reserve(&ACEACCOUNT, 69));
		assert_noop!(
			TreasuryCurrencyAdapter::transfer(&ACEACCOUNT, &ALICE, 69, ExistenceRequirement::AllowDeath),
			Error::<Runtime>::BalanceTooLow,
		);
	});
}
...
fn currency_adapter_refunding_balance_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		let _ = TreasuryCurrencyAdapter::deposit_creating(&ACEACCOUNT, 42);
		Tokens::set_reserved_balance(DOT, &ACEACCOUNT, 69);
		TreasuryCurrencyAdapter::unreserve(&ACEACCOUNT, 69);
		assert_eq!(TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT), 111);
		assert_eq!(TreasuryCurrencyAdapter::reserved_balance(&ACEACCOUNT), 0);
	});
}
...
fn currency_adapter_slashing_balance_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		let _ = TreasuryCurrencyAdapter::deposit_creating(&ACEACCOUNT, 111);
		assert_ok!(TreasuryCurrencyAdapter::reserve(&ACEACCOUNT, 69));
		assert!(TreasuryCurrencyAdapter::slash(&ACEACCOUNT, 69).1.is_zero());
		assert_eq!(TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT), 0);
		assert_eq!(TreasuryCurrencyAdapter::reserved_balance(&ACEACCOUNT), 42);
...
fn currency_adapter_slashing_incomplete_balance_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		let _ = TreasuryCurrencyAdapter::deposit_creating(&ACEACCOUNT, 42);
		assert_eq!(TreasuryCurrencyAdapter::total_issuance(), 42);
		assert_ok!(TreasuryCurrencyAdapter::reserve(&ACEACCOUNT, 21));
		assert_eq!(TreasuryCurrencyAdapter::slash(&ACEACCOUNT, 69).1, 27);
		assert_eq!(TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT), 0);
		assert_eq!(TreasuryCurrencyAdapter::reserved_balance(&ACEACCOUNT), 0);
...
fn currency_adapter_basic_locking_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_eq!(TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT), 100);
			TreasuryCurrencyAdapter::set_lock(ID_1, &ACEACCOUNT, 91, WithdrawReasons::all());
			assert_noop!(
				TreasuryCurrencyAdapter::transfer(&ACEACCOUNT, &ALICE, 10, ExistenceRequirement::AllowDeath),
				Error::<Runtime>::LiquidityRestrictions
			);
		});
}
...
fn currency_adapter_partial_locking_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			TreasuryCurrencyAdapter::set_lock(ID_1, &ACEACCOUNT, 5, WithdrawReasons::all());
...
fn currency_adapter_lock_removal_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			TreasuryCurrencyAdapter::set_lock(ID_1, &ACEACCOUNT, u64::max_value(), WithdrawReasons::all());
			TreasuryCurrencyAdapter::remove_lock(ID_1, &ACEACCOUNT);
			assert_ok!(TreasuryCurrencyAdapter::transfer(
				&ACEACCOUNT,
				&ALICE,
				2,
...
fn currency_adapter_double_locking_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			TreasuryCurrencyAdapter::set_lock(ID_1, &ACEACCOUNT, 5, WithdrawReasons::empty());
			TreasuryCurrencyAdapter::set_lock(ID_2, &ACEACCOUNT, 5, WithdrawReasons::all());
			assert_ok!(TreasuryCurrencyAdapter::transfer(
				&ACEACCOUNT,
				&ALICE,
				2,
...
fn currency_adapter_combination_locking_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			TreasuryCurrencyAdapter::set_lock(ID_1, &ACEACCOUNT, u64::max_value(), WithdrawReasons::empty());
			TreasuryCurrencyAdapter::set_lock(ID_2, &ACEACCOUNT, 0, WithdrawReasons::all());
			assert_noop!(
				TreasuryCurrencyAdapter::transfer(&ACEACCOUNT, &ALICE, 2, ExistenceRequirement::AllowDeath),
				Error::<Runtime>::LiquidityRestrictions
			);
		});
}
...
fn currency_adapter_lock_value_extension_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			TreasuryCurrencyAdapter::set_lock(ID_1, &ACEACCOUNT, 100, WithdrawReasons::all());
			assert_noop!(
				TreasuryCurrencyAdapter::transfer(&ACEACCOUNT, &ALICE, 6, ExistenceRequirement::AllowDeath),
				Error::<Runtime>::LiquidityRestrictions
			);
			TreasuryCurrencyAdapter::extend_lock(ID_1, &ACEACCOUNT, 2, WithdrawReasons::all());
			assert_noop!(
				TreasuryCurrencyAdapter::transfer(&ACEACCOUNT, &ALICE, 6, ExistenceRequirement::AllowDeath),
				Error::<Runtime>::LiquidityRestrictions
			);
			TreasuryCurrencyAdapter::extend_lock(ID_1, &ACEACCOUNT, 8, WithdrawReasons::all());
			assert_noop!(
				TreasuryCurrencyAdapter::transfer(&ACEACCOUNT, &ALICE, 3, ExistenceRequirement::AllowDeath),
				Error::<Runtime>::LiquidityRestrictions
			);
		});
}
...
fn currency_adapter_lock_block_number_extension_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			TreasuryCurrencyAdapter::set_lock(ID_1, &ACEACCOUNT, 200, WithdrawReasons::all());
			assert_noop!(
				TreasuryCurrencyAdapter::transfer(&ACEACCOUNT, &ALICE, 6, ExistenceRequirement::AllowDeath),
				Error::<Runtime>::LiquidityRestrictions
			);
			TreasuryCurrencyAdapter::extend_lock(ID_1, &ACEACCOUNT, 90, WithdrawReasons::all());
			assert_noop!(
				TreasuryCurrencyAdapter::transfer(&ACEACCOUNT, &ALICE, 6, ExistenceRequirement::AllowDeath),
				Error::<Runtime>::LiquidityRestrictions
			);
...
fn currency_adapter_lock_reasons_extension_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			TreasuryCurrencyAdapter::set_lock(ID_1, &ACEACCOUNT, 90, WithdrawReasons::TRANSFER);
			assert_noop!(
				TreasuryCurrencyAdapter::transfer(&ACEACCOUNT, &ALICE, 11, ExistenceRequirement::AllowDeath),
				Error::<Runtime>::LiquidityRestrictions
			);
			TreasuryCurrencyAdapter::extend_lock(ID_1, &ACEACCOUNT, 90, WithdrawReasons::empty());
			assert_noop!(
				TreasuryCurrencyAdapter::transfer(&ACEACCOUNT, &ALICE, 11, ExistenceRequirement::AllowDeath),
				Error::<Runtime>::LiquidityRestrictions
			);
			TreasuryCurrencyAdapter::extend_lock(ID_1, &ACEACCOUNT, 90, WithdrawReasons::RESERVE);
			assert_noop!(
				TreasuryCurrencyAdapter::transfer(&ACEACCOUNT, &ALICE, 11, ExistenceRequirement::AllowDeath),
				Error::<Runtime>::LiquidityRestrictions
			);
...
fn currency_adapter_deposit_creating_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(TreasuryCurrencyAdapter::total_balance(&ACEACCOUNT), 0);
			let _ = TreasuryCurrencyAdapter::deposit_creating(&ACEACCOUNT, 2);
			assert_eq!(TreasuryCurrencyAdapter::total_issuance(), 102);
			assert_eq!(TreasuryCurrencyAdapter::total_balance(&ACEACCOUNT), 2);
...
fn currency_adapter_deposit_into_existing_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(TreasuryCurrencyAdapter::total_balance(&ACEACCOUNT), 0);
			assert_noop!(
				TreasuryCurrencyAdapter::deposit_into_existing(&ACEACCOUNT, 10).map(drop),
				Error::<Runtime>::DeadAccount,
			);
...
fn currency_adapter_reward_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(TreasuryCurrencyAdapter::total_issuance(), 100);
			assert_eq!(TreasuryCurrencyAdapter::total_balance(&ACEACCOUNT), 100);
			assert_ok!(TreasuryCurrencyAdapter::deposit_into_existing(&ACEACCOUNT, 10).map(drop));
			assert_eq!(TreasuryCurrencyAdapter::total_balance(&ACEACCOUNT), 110);
			assert_eq!(TreasuryCurrencyAdapter::total_issuance(), 110);
		});
}
...
fn currency_adapter_slashing_reserved_balance_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(TreasuryCurrencyAdapter::reserve(&ACEACCOUNT, 111));
		assert_eq!(TreasuryCurrencyAdapter::slash_reserved(&ACEACCOUNT, 42).1, 0);
		assert_eq!(TreasuryCurrencyAdapter::reserved_balance(&ACEACCOUNT), 69);
		assert_eq!(TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT), 0);
...
fn currency_adapter_slashing_incomplete_reserved_balance_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		let _ = TreasuryCurrencyAdapter::deposit_creating(&ACEACCOUNT, 111);
		assert_eq!(TreasuryCurrencyAdapter::total_issuance(), 111);
		assert_ok!(TreasuryCurrencyAdapter::reserve(&ACEACCOUNT, 42));
		assert_eq!(TreasuryCurrencyAdapter::slash_reserved(&ACEACCOUNT, 69).1, 27);
		assert_eq!(TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT), 69);
		assert_eq!(TreasuryCurrencyAdapter::reserved_balance(&ACEACCOUNT), 0);
		assert_eq!(TreasuryCurrencyAdapter::total_issuance(), 69);
	});
}
...
fn currency_adapter_repatriating_reserved_balance_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		let _ = TreasuryCurrencyAdapter::deposit_creating(&ACEACCOUNT, 110);
		let _ = TreasuryCurrencyAdapter::deposit_creating(&ALICE, 2);
		assert_ok!(TreasuryCurrencyAdapter::reserve(&ACEACCOUNT, 110));
		assert_ok!(
			TreasuryCurrencyAdapter::repatriate_reserved(&ACEACCOUNT, &ALICE, 41, Status::Free),
			0
		);
		assert_eq!(TreasuryCurrencyAdapter::reserved_balance(&ACEACCOUNT), 69);
		assert_eq!(TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT), 0);
...
fn currency_adapter_transferring_reserved_balance_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(TreasuryCurrencyAdapter::reserve(&ACEACCOUNT, 110));
		assert_ok!(
			TreasuryCurrencyAdapter::repatriate_reserved(&ACEACCOUNT, &ALICE, 41, Status::Reserved),
			0
		);
		assert_eq!(TreasuryCurrencyAdapter::reserved_balance(&ACEACCOUNT), 69);
		assert_eq!(TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT), 0);
...
fn currency_adapter_transferring_incomplete_reserved_balance_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		let _ = TreasuryCurrencyAdapter::deposit_creating(&ACEACCOUNT, 110);
		let _ = TreasuryCurrencyAdapter::deposit_creating(&ALICE, 2);
		assert_ok!(TreasuryCurrencyAdapter::reserve(&ACEACCOUNT, 41));
		assert_ok!(
			TreasuryCurrencyAdapter::repatriate_reserved(&ACEACCOUNT, &ALICE, 69, Status::Free),
			28
		);
		assert_eq!(TreasuryCurrencyAdapter::reserved_balance(&ACEACCOUNT), 0);
		assert_eq!(TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT), 69);
...
fn currency_adapter_transferring_too_high_value_should_not_panic() {
	ExtBuilder::default().build().execute_with(|| {
		TreasuryCurrencyAdapter::make_free_balance_be(&ACEACCOUNT, u64::max_value());
		...
		assert_eq!(
			TreasuryCurrencyAdapter::free_balance(&ACEACCOUNT),
			u64::max_value()
		);
		assert_eq!(TreasuryCurrencyAdapter::free_balance(&ALICE), 2);
	});
